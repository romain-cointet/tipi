...
heroNumbers.innerHTML +=
  students.length + " étudiants, " + jobs.length + " jobs";
...
for (let i = 0; i < jobs.length; i++) {
  ...
  content +=
    '<div id="job-offer"><h5 id="job-enterprise">' +
    jobs[i].enterprise +
    '</h5> <div><p class="job-salary">' +
    jobs[i].salary +
    "</p> </div></div>";
  content += "</div>";
  item.innerHTML = content;
...
let registeredPlacesToVisit = document.getElementById(
  "registered-places-to-visit"
);
registeredPlacesToVisit.innerHTML =
  places.length + " lieux à visiter dans la ville ";

let registeredPlacesWrapper = document.getElementById(
  "registered-places-wrapper"
);
...
for (let i = 0; i < places.length; i++) {
  ...
  content =
    '<div class="places-img">  <img src=" ' +
    places[i].url +
    '" alt="' +
    '"></div>';
  content += '<div id="places-titles">' + places[i].name + "</div>";
  content +=
    '<div id="place"><h5 id="location">' +
    places[i].place +
    '</h5> <div><p id="place-fee">' +
    places[i].price +
    "</p> </div>";
  content += "</div>";
  item.innerHTML = content;
